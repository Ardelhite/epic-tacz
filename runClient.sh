...
resolve_java_home() {
    # 1. Honor caller-provided JAVA_HOME if it actually is JDK 21.
    ...
    # 3. Homebrew openjdk@21 (Apple Silicon then Intel prefix).
    for p in \
    ...
    # 4. Anything matching jdk-21 under /Library/Java.
...
if jh=$(resolve_java_home); then
    ...
    echo "runClient.sh: could not find a JDK 21 install." >&2
